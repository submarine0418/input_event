...
### Automatic Attachment with udev

Create `/etc/udev/rules.d/99-arduino-joystick.rules`:
```bash
ACTION=="add", SUBSYSTEM=="tty", ATTRS{idVendor}=="2341", ATTRS{idProduct}=="0043", RUN+="/usr/sbin/ldattach -d -s 9600 29 /dev/%k"
```
...
echo "serial_wasd_ldisc" | sudo tee -a /etc/modules

# For systemd auto-attach, create a service
sudo nano /etc/systemd/system/arduino-joystick.service
```

## 🐛 Troubleshooting

### Issue: `ldattach` returns "Invalid argument"

**Cause**: The TTY device is in an inconsistent state.

**Solution**:
```bash
# Unplug Arduino, wait 5 seconds, replug, then retry
sudo ldattach -d -s 9600 29 /dev/ttyACM0
```

### Issue: Driver loads but no input events

**Symptoms**: `dmesg` shows "open successful" but no data received.

**Checklist**:
1. ✅ Arduino is sending heartbeat (`.`) every ~1 second
2. ✅ Baud rate matches (9600)
3. ✅ Check `dmesg` for "Received X bytes" messages
4. ✅ Verify with `sudo cat /dev/ttyACM0` (should see dots)

**Solution**: The current code already handles this via `tty_set_termios()` and `receive_room` settings.

### Issue: System freezes or kernel panic

**Cause**: Memory corruption or race condition.

**Solution**: Ensure you're using the latest version with:
- `tty->disc_data` for state management (not global variables)
- Spinlock protection in `receive_buf`
- Proper cleanup in `close`

### Issue: Detaching doesn't work cleanly

**Solution**:
```bash
# Kill ldattach process
